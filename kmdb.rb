...
christian_bale = Actor.create(name: "Christian Bale", agent: "Yes")
...
for movie in movies
  puts "#{movie["title"].ljust(23)}#{movie["year_released"].to_s.ljust(15)}#{movie["rated"].ljust(7)}#{movie["studio"]["name"]}"
end
...
for role in roles
  puts "#{role["movie"]["title"].ljust(23)}#{role["actor"]["name"].ljust(22)}#{role["character_name"]}"
end
